...
void get_cfg_dat_JOIN_INNER_TD_263_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_INNER_TD_263_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((p_partkey#258 = l_partkey#78))
    // Left Table: ListBuffer(p_partkey#258)
...
void get_meta_info_JOIN_INNER_TD_263_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
...
void get_meta_info_JOIN_INNER_TD_263_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_INNER_TD_263_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
