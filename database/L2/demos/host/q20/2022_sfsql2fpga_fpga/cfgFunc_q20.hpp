...
void get_cfg_dat_JOIN_LEFTSEMI_TD_6400_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_6400_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_LEFTSEMI
    // Operation: ListBuffer((l_partkey#78 = p_partkey#258))
    // Left Table: ListBuffer(p_partkey#258)
...
void get_meta_info_JOIN_LEFTSEMI_TD_6400_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
...
void get_meta_info_JOIN_LEFTSEMI_TD_6400_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_6400_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_487_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_LEFTSEMI
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_487_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_LEFTSEMI
    // Operation: ListBuffer((ps_partkey#305 = p_partkey#258))
    // Left Table: ListBuffer(p_partkey#258)
...
void get_meta_info_JOIN_LEFTSEMI_TD_487_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_meta_info_JOIN_LEFTSEMI_TD_487_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_487_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
...
void get_cfg_dat_JOIN_INNER_TD_3971_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
...
void get_cfg_dat_JOIN_INNER_TD_3971_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((((cast(ps_availqty#307 as bigint) > sum(l_quantity)#1848L) AND (l_partkey#78 = ps_partkey#305)) AND (l_suppkey#79 = ps_suppkey#306)))
    // Left Table: ListBuffer(ps_partkey#305, ps_suppkey#306, ps_availqty#307)
...
void get_meta_info_JOIN_INNER_TD_3971_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_meta_info_JOIN_INNER_TD_3971_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_INNER_TD_3971_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_2112_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_2112_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_LEFTSEMI
    // Operation: ListBuffer((s_suppkey#342 = ps_suppkey#306))
    // Left Table: ListBuffer(ps_suppkey#306)
...
void get_meta_info_JOIN_LEFTSEMI_TD_2112_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_meta_info_JOIN_LEFTSEMI_TD_2112_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_LEFTSEMI_TD_2112_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
...
void get_cfg_dat_JOIN_INNER_TD_1763_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
...
void get_cfg_dat_JOIN_INNER_TD_1763_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((s_nationkey#345 = n_nationkey#144))
    // Left Table: ListBuffer(s_name#343, s_address#344, s_nationkey#345)
...
void get_meta_info_JOIN_INNER_TD_1763_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_meta_info_JOIN_INNER_TD_1763_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_INNER_TD_1763_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
