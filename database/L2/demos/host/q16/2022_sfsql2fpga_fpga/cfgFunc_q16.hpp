...
void get_cfg_dat_JOIN_LEFTANTI_TD_3403_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_LEFTANTI_TD_3403_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_LEFTANTI
    // Operation: ListBuffer(((ps_suppkey#306 = s_suppkey#342) OR isnull((ps_suppkey#306 = s_suppkey#342))))
    // Left Table: ListBuffer(s_suppkey#342)
...
void get_meta_info_JOIN_LEFTANTI_TD_3403_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
...
void get_meta_info_JOIN_LEFTANTI_TD_3403_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_LEFTANTI_TD_3403_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
...
void get_cfg_dat_JOIN_INNER_TD_2163_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_INNER_TD_2163_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((p_partkey#258 = ps_partkey#305))
    // Left Table: ListBuffer(p_partkey#258, p_brand#261, p_type#262, p_size#263)
...
void get_meta_info_JOIN_INNER_TD_2163_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_meta_info_JOIN_INNER_TD_2163_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_INNER_TD_2163_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;

    // input table a
    signed char id_a[] = {0,3,1,2,-1,-1,-1,-1};
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }
    // filter tbl_a config
