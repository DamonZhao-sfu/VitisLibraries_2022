...
void get_cfg_dat_JOIN_INNER_TD_314_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_INNER_TD_314_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((c_custkey#22 = o_custkey#208))
    // Left Table: ListBuffer(c_custkey#22)
...
void get_meta_info_JOIN_INNER_TD_314_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
...
void get_meta_info_JOIN_INNER_TD_314_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
...
void get_cfg_dat_JOIN_INNER_TD_314_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
...
void get_cfg_dat_JOIN_INNER_TD_2246_gqe_join(ap_uint<512>* hbuf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
...
void get_cfg_dat_JOIN_INNER_TD_2246_gqe_join_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
...
void get_cfg_dat_Aggregate_TD_133_gqe_aggr(ap_uint<32>* buf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
...
void get_cfg_dat_Aggregate_TD_133_gqe_aggr_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
