...
void get_cfg_dat_Aggregate_TD_4156_gqe_aggr(ap_uint<32>* buf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: Aggregate
    // Operation: ListBuffer(l_suppkey#79, sum((l_extendedprice#82 * (1 - l_discount#83))) AS sum((l_extendedprice * (1 - l_discount)))#1456L)
    // Input Table: ListBuffer(l_suppkey#79, l_extendedprice#82, l_discount#83)
    // Output Table: ListBuffer(l_suppkey#79, sum((l_extendedprice * (1 - l_discount)))#1456L)
    // Node Depth: 4
    ...
    shuffle1_cfg(31, 24) = 8; // sum((l_extendedprice * (1 - l_discount)))#1456L
    ...
    shuffle2_cfg(31, 24) = 3; // sum((l_extendedprice * (1 - l_discount)))#1456L
    ...
    shuffle4_cfg(7, 0) = 3; // sum((l_extendedprice * (1 - l_discount)))#1456L
    shuffle4_cfg(15, 8) = -1;
...
void get_cfg_dat_Aggregate_TD_4156_gqe_aggr_part(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;

    // input table a
    signed char id_a[] = {2,3,-1,-1,-1,-1,-1,-1};
    for (int c = 0; c < 8; ++c) {
