...
void get_cfg_dat_JOIN_INNER_TD_2493_gqe_join_build(xf::database::gqe::KernelCommand& krn_cmd) {
...
void get_cfg_dat_JOIN_INNER_TD_2493_gqe_join_probe(xf::database::gqe::KernelCommand& krn_cmd) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((o_orderkey#207 = l_orderkey#77))
    // Left Table: ListBuffer(l_orderkey#77, l_shipmode#91)
...
void get_meta_info_JOIN_INNER_TD_2493_gqe_join_build(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
...
void get_meta_info_JOIN_INNER_TD_2493_gqe_join_probe(MetaTable* meta, int nrow, int ncol) {
    meta->setSecID(0);
    meta->setColNum(ncol);
    for(int i=0; i<ncol; i++){
         meta->setCol(i,i,nrow);
